...
def plot_subspace_distances(U_Gr_list, U_Fl, signature):
    plt.figure()
    plt.plot([subspace_distance(U_Gr_list[k], U_Gr_list[k+1]) for k in range(len(signature) - 1)], linewidth=3, color=(179/255, 21/255, 41/255), label="Grassmann")
    plt.plot([subspace_distance(U_Fl[:, :signature[k]], U_Fl[:, :signature[k+1]]) for k in range(len(signature) - 1)], linewidth=3, color=(16/255, 101/255, 171/255), label="Flag")
    plt.legend()
    plt.show(block=False)


def plot_nestedness_scatter(X, U_Gr_1, U_Gr_2, U_Fl, colors):
    n = X.shape[1]
    fig, axes = plt.subplots(nrows=2, ncols=2, sharex=True, sharey=True)
    X_subspace_1 = U_Gr_1.T @ X
    axes[0, 0].scatter(X_subspace_1, np.zeros(n,), c=colors, alpha=1)
    axes[0, 0].set_title('Subspace - 1D')
    X_subspace_2 = U_Gr_2.T @ X
    axes[1, 0].scatter(*X_subspace_2, c=colors, alpha=1)
    axes[1, 0].set_title('Subspace - 2D')
    X_flag = U_Fl.T @ X
    axes[0, 1].scatter(X_flag[0], np.zeros(n,), c=colors, alpha=1)
    axes[0, 1].set_title('Flag - 1D')
    axes[1, 1].scatter(X_flag[0], X_flag[1], c=colors, alpha=1)
...
def plot_explained_variance(X, U_Gr_list, U_Fl, signature):
    ...
    plt.figure()
    plt.plot((0,)+signature+(p,), [np.trace(U.T @ X @ X.T @ U) / np.trace(X @ X.T) for U in U_Gr_list], linewidth=3, color=(179/255, 21/255, 41/255), label='Grassmann')
    plt.plot((0,)+signature+(p,), [np.trace(U.T @ X @ X.T @ U) / np.trace(X @ X.T) for U in U_Fl_list], linewidth=3, color=(16/255, 101/255, 171/255), label='Flag')
    plt.legend()
    plt.show(block=False)
...
def plot_reconstruction_errors(X, n_in, U_Gr, U_Fl, signature):
    ...
    plt.figure()
    plt.scatter(np.arange(n)[labels_gr==0], reconstruction_errors_gr[labels_gr==0], label='inliers', color=(16/255, 101/255, 171/255), alpha=1)
    plt.scatter(np.arange(n)[labels_gr==1], reconstruction_errors_gr[labels_gr==1], label='outliers', color=(179/255, 21/255, 41/255), alpha=1)
    plt.plot(np.arange(n), reconstruction_errors_gr, color='k', label='reconstruction errors')
    plt.title("Gr(64, 5)")
    plt.legend()
    plt.show(block=False)
    plt.figure()
    plt.scatter(np.arange(n)[labels_fl==0], reconstruction_errors_fl[labels_fl==0], label='inliers', color=(16/255, 101/255, 171/255), alpha=1)
    plt.scatter(np.arange(n)[labels_fl==1], reconstruction_errors_fl[labels_fl==1], label='outliers', color=(179/255, 21/255, 41/255), alpha=1)
...
def plot_nestedness_scatter_sc(U_Gr_1, U_Gr_2, U_Fl, colors):
    n = U_Gr_1.shape[0]
    fig, axes = plt.subplots(nrows=2, ncols=2, sharex=True, sharey=True)
    axes[0, 0].scatter(U_Gr_1[:, 0], np.zeros(n,), c=colors, alpha=1)
    axes[0, 0].set_title('Subspace - 1D')
    axes[1, 0].scatter(U_Gr_2[:, 0], U_Gr_2[:, 1], c=colors, alpha=1)
    axes[1, 0].set_title('Subspace - 2D')
    axes[0, 1].scatter(U_Fl[:, 0], np.zeros(n,), c=colors, alpha=1)
    axes[0, 1].set_title('Flag - 1D')
    axes[1, 1].scatter(U_Fl[:, 0], U_Fl[:, 1], c=colors, alpha=1)
    axes[1, 1].set_title('Flag - 2D')
    plt.show(block=False)
    fig, axes = plt.subplots(nrows=2, ncols=2, sharex=True, sharey=True)
    plt.set_cmap("Accent")
    axes[0, 0].scatter(U_Gr_1/np.linalg.norm(U_Gr_1, axis=1)[:, np.newaxis], np.zeros(n,), c=colors, alpha=1)
    axes[0, 0].set_title('Subspace - 1D')
    axes[1, 0].scatter(*(U_Gr_2/np.linalg.norm(U_Gr_2, axis=1)[:, np.newaxis]).T, c=colors, alpha=1)
    axes[1, 0].set_title('Subspace - 2D')
    axes[0, 1].scatter(U_Fl[:, :1]/np.linalg.norm(U_Fl[:, :1], axis=1)[:, np.newaxis], np.zeros(n,), c=colors, alpha=1)
    axes[0, 1].set_title('Flag - 1D')
    axes[1, 1].scatter(*(U_Fl[:, :2]/np.linalg.norm(U_Fl[:, :2], axis=1)[:, np.newaxis]).T, c=colors, alpha=1)
    axes[1, 1].set_title('Flag - 2D')
    plt.show(block=False)


def plot_scatter_3D(X, colors, U_Gr_list=None, U_Fl=None, length=5):
    fig = plt.figure(figsize=(7, 7))
    ax1 = fig.add_subplot(111, projection='3d')
    ax1.scatter(*X, c=colors, alpha=1)
...
def color(y):
    if y == 0:
        return 16/255, 101/255, 171/255
    elif y == 1 / 4:
        return 142/255, 196/255, 222/255
    elif y == 2 / 4:
        return 200/255, 200/255, 200/255
    elif y == 3 / 4:
        return 246/255, 164/255, 130/255
    elif y == 1:
        return 179/255, 21/255, 41/255
    else:
